...
import {POLLUTION_CLASSES, POLLUTION_NUM_CLASSES} from './data';
...
export function renderEvaluateTable(xData, yTrue, yPred, logits) {
  ...
  for (let i = 0; i < yTrue.length; ++i) {
    ...
    truthCell.textContent = POLLUTION_CLASSES[yTrue[i]];
    row.appendChild(truthCell);
    const predCell = document.createElement('td');
    predCell.textContent = POLLUTION_CLASSES[yPred[i]];
    predCell.classList =
        yPred[i] === yTrue[i] ? ['correct-prediction'] : ['wrong-prediction'];
    row.appendChild(predCell);
    const logitsCell = document.createElement('td');
    const exampleLogits =
        logits.slice(i * POLLUTION_NUM_CLASSES, (i + 1) * POLLUTION_NUM_CLASSES);
